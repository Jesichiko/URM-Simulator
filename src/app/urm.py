...
class URMExecutorGUI:
    def __init__(self, root):
        ...
        style.configure(
            "Surface.TFrame", background=surface_dark, borderwidth=1, relief="solid"
        )
        style.configure(
            "Dark.TLabel",
        ...
        self.code_input.grid(
            row=1, column=0, sticky=(tk.W, tk.E, tk.N, tk.S), padx=(0, 12)
        )
        ...
        self.trace_output.grid(
            row=1, column=1, sticky=(tk.W, tk.E, tk.N, tk.S), padx=(12, 0)
        )
    ...
    def execute_program(self):
        try:
            ...
            # Si la entrada esta vacia, usar lista vacia []
            if not input_str:
                program_input = []
            else:
                try:
                    # Parseamos la entrada como numeros separados por comas
                    program_input = [int(x.strip()) for x in input_str.split(",")]
                except ValueError:
                    messagebox.showerror(
                        "Error",
                        "La entrada debe ser numeros separados por comas, ej: 5, 2, 0, o dejarla vacia para []",
                    )
                    return
